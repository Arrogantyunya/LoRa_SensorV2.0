...
{ 
  Which_LED = which_led;
  LED_Freq = freq;
  Timer4.setChannel1Mode(TIMER_OUTPUTCOMPARE);//设置通道模式
  Timer4.setPeriod(100000); // in microseconds, 100ms设置规定时间
  Timer4.setCompare1(1);//设置比较
...
unsigned int Some_Peripherals::Get_Voltage(void)
{
  ...
  //Serial.println("电压采采采采采采集集集集集集集");
