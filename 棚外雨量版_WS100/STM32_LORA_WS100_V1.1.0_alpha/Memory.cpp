...
bool Work_Parameter_Information::Save_Collect_Time(unsigned int time)
{
	...
	if (Time < 30 || Time > 43200)
	{
		Time = 180;
		Serial1.println("Time < 30 || Time > 43200,采集时间超出阈值!!!Time = 180");
	}
    ...
    if (Time_Temp == Time)
	{
        Serial1.println("collect data already stored收集已存储的数据...");
		Serial1.println(String("服务器设置的合法time_temp = ") + Time_Temp);
        return true;
    }
	else
	{
