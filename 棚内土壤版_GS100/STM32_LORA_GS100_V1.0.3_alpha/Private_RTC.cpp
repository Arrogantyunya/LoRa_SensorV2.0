...
void date::Set_Alarm(void)
{
	...
	if (WorkParameter_Info.Verify_Collect_Time_Flag())
	{
		...
		if (LowBalFlag) //如果电压过低，强制休眠20分钟
		{
			...
			{
				alarm += Time_temp;
				Serial.println(String("低电压！！！强制休眠")+String(Time_temp)+"秒。");
			}
			else
			{
				alarm += 1200;
				Serial.println("低电压！！！强制休眠20分钟");
			}
		}
		else
		{
			alarm += Time_temp;
			Serial.println(String("服务器设置休眠") + String(Time_temp) + "秒。");
		}
	}
	else
	{
		if (LowBalFlag)
		{
			...
			Serial.println("没有得到时间值，低电量强制休眠20分钟");
		}
		else
		{
			alarm += 180;
			Serial.println("没有得到时间值，休眠3分钟");
		}
