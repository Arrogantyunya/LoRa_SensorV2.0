...
#define USE_GATEWAY_MODE 0

//GPIO definition
#define RESET_PIN       PA5
#define WAKEUP_PIN      PA4
#define AT_CMD_PIN      PA0
...
public:
    ...
    void Mode(LoRa_Mode AT_status);
    void IsReset(bool Is_reset);
    bool LoRa_AT(unsigned char *data_buffer, bool is_query, char *cmd, char *para);

    void Parameter_Init(void);

private:
    unsigned char Detect_Error_Receipt(unsigned char *verify_data, unsigned char *data_buffer);
    bool Detect_OK_Receipt(unsigned char *verify_data, unsigned char *data_buffer);
    //Read LoRa parameters.
    Receive_Type AT_Query_Cmd(char *cmd, unsigned char *data_buffer, unsigned char *data_len);
    //Set LoRa parameters.
    Receive_Type AT_Config_Cmd(char *cmd, char * para, unsigned char *data_buffer);
    bool Parse_Command(unsigned char *addr_temp, unsigned char len, unsigned char *data_buffer, unsigned char **data_len);
    //Get the LoRa return information
    void Get_Bytes(unsigned char *addr_temp, unsigned char len, unsigned char *data_buffer);
    //SNR and RSSI
