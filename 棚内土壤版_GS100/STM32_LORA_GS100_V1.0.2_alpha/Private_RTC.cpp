...
void RTC_Interrupt(void)
{
	...
	//Serial.begin(9600);
	//Serial.println("shiyan");

	rtc_detach_interrupt(RTC_ALARM_SPECIFIC_INTERRUPT);//a禁止RTC闹钟中断
	PWR_ClearFlag(PWR_FLAG_WU);//c将PWR唤醒（WUF）标志清零
	PWR_ClearFlag(PWR_FLAG_SB);//c将PWR唤醒（WUF）标志清零
	nvic_sys_reset();
}
...
void date::Set_Alarm(void)
{
	...
	else
	{
		...
		{
			alarm += 10;
			Serial.println(String("没有收到服务器的参数设置！！电池电压正常，自动修订的间隔休眠时间为：Time_temp = 10s"));
		}
	}

	Serial.println("abcd");

	Date.createAlarm(RTC_Interrupt, alarm);//d使能RTC时钟中断
	delay(10);
	//bkp_disable_writes();
}
